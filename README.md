...
[![Online Imputation Simulation](https://img.youtube.com/vi/7xL2HA_SC9U/maxresdefault.jpg)](https://www.youtube.com/watch?v=7xL2HA_SC9U)
...
│   ├── TelemetriaConsumosVilaSol_v2.csv  # Static input
│   └── inputs_ts.csv            # ⚙️ Generated by notebook
│
├── variables
│   └── variables.pkl            # ⚙️ Generated by notebook
│
└── README.md
```
...
`seq_all_df` → exported as `inputs_ts.csv`
...
### Group Level
`(tipo_consumo, calibre)` — e.g., ("Doméstico", 15)

### Subgroup Level
`(month, day_of_week, hour)` — a specific temporal context within a group.

### Data Structure

The clustering output is a **list of lists**. Each index corresponds to one group and contains a list of dictionaries — one per subgroup. Each dictionary has the following keys:
...
- `α` (`ewma_alpha`): controls how fast the belief adapts to new observations
...
- `x̄(t)_subgroup`: mean consumption across all LCs in that subgroup at time `t`
